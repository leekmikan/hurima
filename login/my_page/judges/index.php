		...
		<body>
			<?php
				...
				if (isset($_SESSION['id'])) {
					...
					$stmt = $mysqli->query('SELECT * FROM items WHERE user_id = "'.$_SESSION['id'].'" AND judge IS NOT NULL '.$lim.';');
                    $stmt2 = $mysqli->query('SELECT judge FROM items WHERE user_id = "'.$_SESSION['id'].'" AND judge IS NOT NULL;');
					$hit = mysqli_num_rows($stmt2);
				}else{
					header('Location:../../../login/index.php');exit;
				}
			?>
			<header>
				<h1 id="title">あなたの評価</h1>
			</header>
			<div class="main">
					<?php
					if (isset($_SESSION['id'])) {
                        ...
                        $avr = 0;
                        $sd = 0;
                        while($judge = $stmt2->fetch_array(MYSQLI_ASSOC)){
                        ...
                        $sd = round(sqrt($sd / $hit)  * 10) / 10;
					    echo '<h2 class="ct">平均評価：'.$avr.' / 5.0</h2>';
                        echo '<h2 class="ct">標準偏差：'.$sd.'</h2>';
                        for($i = 0;$i < count($dist);$i++){
                            echo '<p>☆'.($i + 1).'：'.$dist[$i].' 件</p>';
                        }
